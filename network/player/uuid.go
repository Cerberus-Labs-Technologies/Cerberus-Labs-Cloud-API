...
// scan and value implement the sql.Scanner and driver.Valuer interfaces
func (i *UUID) Scan(src interface{}) error {
	switch src := src.(type) {
	case []byte:
		u, err := guuid.Parse(string(src))
		if err != nil {
			log.Println("Error while scanning UUID: " + err.Error())
			return err
		}
		*i = UUID(u)
	case string:
		u, err := guuid.Parse(src)
		if err != nil {
			return err
		}
		*i = UUID(u)
	default:
		return fmt.Errorf("Unsupported data type for UUID: %T", src)
	}
	return nil
}

func (i UUID) Value() (driver.Value, error) {
	return i.String(), nil
}
